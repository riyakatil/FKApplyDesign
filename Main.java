...
class Squarecell implements cellInterface{
    ...
    public Marker getMarker(){return this.m1;}

    public void updateCell(Marker ch){ this.m1=ch;}
...
interface BoardInterface
{
    ...
    void updateBoard(int x,int y,Marker m1);
    boolean checkIfBoardIsFull();
    ArrayList<cellInterface[]> getBoardPositions();

    Marker getMarkerAtPosition(int x,int y);
...
class SquareBoard implements BoardInterface{
    ...
    public void fillBoard()
    {
        ...
        for(int i=0;i<this.getDepth();i++)
        {
            for(int j=0;j<this.getDepth();j++)
            {
                ...
                cellInterface tmp1=new Squarecell(tmp);
    ...
    public void updateBoard(int x,int y,Marker ch){
    ...
    public boolean checkIfBoardIsFull(){
        ...
        for(int i=0;i<this.getDepth();i++){
            for(int j=0;j<this.getDepth();j++){
                ...
                Marker tmp=arr.get(i)[j].getMarker();
                if(!tmp.equals(Marker.X) && !tmp.equals(Marker.O)  && !tmp.equals(Marker.NoCell)) {flag=  false; };
            }
        }
    ...
    public Marker getMarkerAtPosition(int x,int y){
        return arr.get(x)[y].getMarker();
    }
...
class HexBoard implements BoardInterface{
    ...
    public void fillBoard()
    {
        ...
        Marker t1=Marker.NoCell;
        for(int i=0;i<this.getDepth();i++)
        {
            boolean flag=true;
            for(int j=0;j<this.getDepth();j++)
            {
                if(i%2==0){ if(flag){t1=Marker.NoCell;} else {t1=Marker.EMPTY;}}
                else{
                    if(flag){t1=Marker.EMPTY;} else {t1=Marker.NoCell;}
                } flag=!flag;

                cellInterface tmp=new HexCell(t1);
                arr.get(i)[j]=tmp;
...
class JudgeHex implements JudgeInterface{
    ...
    private int consecutiveChar(ArrayList<Marker> a1,Marker m1)
...
//check in this diagonal
        ...
        while(cur_x<row && cur_y<row){
            a1.add(ar1.get(cur_x)[cur_y].getMarker());
            cur_x++;
            cur_y++;
        }

        int countX=consecutiveChar(a1,Marker.X);
        int countO=consecutiveChar(a1,Marker.O);
...
//check in opposite of diagon ally
        ...
        countX=consecutiveChar(a1,Marker.X);
        countO=consecutiveChar(a1,Marker.O);
...
//Anti Diagonally
    ...
    private boolean diagonalCrossed(ArrayList<cellInterface[]> ar1)
    {
        ...
        for (int i=0; i<row; i++)
        {
            ...
            if(ar1.get(i)[i].getMarker().equals(Marker.X)){countX++ ;}
            else if(ar1.get(i)[i].getMarker().equals(Marker.O)){countO++ ;}
        ...
        for (int i=row-1; i>=0; i--)
        {
            ...
            if(ar1.get(row-1-i)[i].getMarker().equals (Marker.X)){countX++ ;}
            else if(ar1.get(row-1-i)[i].getMarker().equals(Marker.O)){countO++ ;}
    ...
    private boolean columnCrossed(ArrayList<cellInterface[]> ar1)
    {
        ...
        for (int i=0; i<col; i++)
        {
            ...
            for(int j=0;j<row;j++){
                ...
                if(ar1.get(j)[i].getMarker().equals( Marker.X)){countX++ ;}
                else if(ar1.get(j)[i].getMarker().equals( Marker.O)){countO++ ;}
            }
    ...
    private boolean rowCrossed(ArrayList<cellInterface[]> ar1)
    {
        ...
        for (int i=0; i<row; i++)
        {
            ...
            for(int j=0;j<col;j++){
                ...
                if(ar1.get(i)[j].getMarker().equals( Marker.X)){countX++ ;}
                else if(ar1.get(i)[j].getMarker().equals( Marker.O)){countO++ ;}
            }
            if(countX==row ||  countO==row) {flag=true; break;}
        ...
        while(true){
            ...
            while(!this.checkIfBoardIsFull())
            {
                ...
                if(whoseTurn instanceof Human ){
                    s="Do you want to reverse? enter 1 to undo ";
                    this.printMessage(s);
...
//depth of board
    ...
    public void printBoardStatus()
    {
        ...
        {
            for(int j=0;j<col;j++)
            {    Marker tmp=b1.getMarkerAtPosition(i,j);
                if(tmp==Marker.NoCell){
                        System.out.print('*'+" ");
                }
                else if(tmp==Marker.EMPTY){
                     System.out.print('_'+" ");   
                }
                else{
    ...
    public Marker getMarkerAtPosition(int i,int j){
        return (Marker)this.b1.getMarkerAtPosition(i,j);
    }
...
//check board is full
    ...
    //print message
    private void printMessage(Player whoseTurn,int []pos,Marker ch){
    ...
    private boolean checkIfThisIsValidPosition(int[] pos){
        int i=pos[0];
        int j=pos[1];
        Marker tmp=getMarkerAtPosition(i,j);
        if(i>=0 && j>=0 && i<this.getDepthOfBoard() && j<this.getDepthOfBoard() && !tmp.equals(Marker.X) && !tmp.equals(Marker.O)  && !tmp.equals(Marker.NoCell)) {return true;}
        return false;
    ...
    public int[] getFreeCellOfBoard(){
        ...
        for(int i=0;i<this.getDepthOfBoard();i++)
        {
            ...
            {
                Marker tmp=getMarkerAtPosition(i,j);
                if(!tmp.equals(Marker.X) && !tmp.equals(Marker.O)  && !tmp.equals(Marker.NoCell) )
                {
                    pos[0]=i;
                    pos[1]=j;
    ...
    //update
    private void updateBoard(int[]pos,Marker ch){
